...
| **Accessibilité RGAA** — audit **complet** du 27 août 2026 (106 critères, aucun sans verdict, **86,2 %**), **schéma pluriannuel 2026-2029** et plan d'action publiés dans l'app, **9 non-conformités** restantes ; ⚠️ un contrôle qui ne mesure rien **ne rougit pas, il verdit** (axe sur un écran encore invisible ; repère de JSON introuvable dans `validite-html.yml`) | `docs/accessibilite/audit-rgaa-2026-08-27.md` et `docs/accessibilite/schema-pluriannuel.md`, puis `docs/adr/0030-axe-mesure-un-ecran-encore-invisible.md` |
| **Validité HTML (RGAA 8.2)** — `vnu` en local, une seule issue vivante, ⚠️ **ne fait pas échouer le build** (33 erreurs connues) ; corriger les 28 tuiles impose de rendre le `display:block` que la balise `<div>` fournissait | `.github/workflows/validite-html.yml` puis l'audit §« Cinquième passe » |
