...
| **Fusion automatique Dependabot** (`dependabot-auto-merge.yml`) — ⛔ **jamais une majeure, et jamais une mineure en 0.x** (en pré-1.0 c'est la mineure qui porte les ruptures, semver §4) ; ⛔ **« aucune » n'est pas « verte »** : sans check ayant tourné, on ne fusionne pas ; ⛔ la PR doit ne toucher **que** `package.json`/`package-lock.json`, et l'auteur est lu **sur l'API** (un titre, ça s'écrit) ; ⚠️ **pas d'`--auto` natif** — `main` n'étant pas protégée, l'auto-merge GitHub fusionnerait **sans attendre la CI** ; ⚠️ le `workflow_run` n'est qu'un **réveil**, la décision se prend sur l'état **global** des checks | `docs/adr/0045-fusionner-une-mise-a-jour-de-version-sans-relecture.md` puis `docs/guide-technique.md` §10 |
