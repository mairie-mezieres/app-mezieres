...
| **Accessibilité RGAA** — audit **complet** (106 critères, aucun sans verdict, **96,9 %** en v4.93), **schéma pluriannuel 2026-2029** et plan d'action publiés dans l'app, **2 non-conformités** restantes (3.2 et 10.5) ; ⚠️ un contrôle qui ne mesure rien **ne rougit pas, il verdit** (axe sur un écran encore invisible ; repère de JSON introuvable dans `validite-html.yml`) | `docs/accessibilite/audit-rgaa-2026-08-27.md` et `docs/accessibilite/schema-pluriannuel.md`, puis `docs/adr/0030-axe-mesure-un-ecran-encore-invisible.md` |
| **Contraste (RGAA 3.2)** — ⛔ trois pièges qui font verdir un contrôle faux : un `rgba(255,255,255,.72)` **n'est pas du blanc** (compositer avant de comparer), un **dégradé** se mesure sur tout son continuum (`#38bdf8` échouait quand `#2563eb` passait, sur la même carte), et un **voile clair** sur fond sombre *éclaircit* le fond — d'où des voiles **sombres** sur les pastilles et les panneaux. Les libellés « Bac noir/jaune » ne portent plus la couleur : les **pastilles rondes** le font. Le voile du hero bureau est calculé pour le **pire cas absolu**, jamais sur la photo du jour | `docs/accessibilite/audit-rgaa-2026-08-27.md` §« Huitième passe », puis `tests/e2e/contraste-bandeaux.spec.js` et `tests/e2e/contraste-global.spec.js` |
