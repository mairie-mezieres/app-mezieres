...
function _buildTriCard() {
  ...
  _TRI_DATA.forEach(function(b) {
    html += '<div style="border:1px solid ' + b.border + ';border-radius:10px;margin-bottom:8px;overflow:hidden">'
      ...
      + '<span class="tri-chev" id="tri-chev-' + b.id + '" style="font-size:0.65rem">▼</span>'
      + '</button>'
...
async function toggleDechetsNotif() {
  ...
  if (enabled) {
    try {
      ...
      var sub = await reg.pushManager.getSubscription();
      if (sub) {
        fetch(window.MAT_API+'/push/unsubscribe/dechets', {
          method: 'POST', headers: { 'Content-Type': 'application/json' },
  ...
  try {
    ...
    await fetch(window.MAT_API+'/push/subscribe/dechets', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
